...
const handler = async (req: string, res: HomePost) => {
  try {
    ...
    const promise1 = databases.listDocuments(`${CMS_DB_ID}`, `${CURRENT_ID}`, [
      Query.orderDesc("$createdAt"), // Order by title in ascending order
      // Order by year in descending order
    ]);

    const promise2 = databases.listDocuments(`${CMS_DB_ID}`, `${SPORTS_ID}`, [
      Query.orderDesc("$createdAt"), // Order by title in ascending order
      // Order by year in descending order
    ]);
    const promise3 = databases.listDocuments(`${CMS_DB_ID}`, `${ECO_ID}`, [
      Query.orderDesc("$createdAt"), // Order by title in ascending order
      // Order by year in descending order
    ]);
